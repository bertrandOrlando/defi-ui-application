          ...
          {agreementData.map((row) => (
            ...
            >
              ...
              <TableCell sx={cellStyle}>
                <StatusBubble status={row.verificationStatus} />
              </TableCell>
              <TableCell sx={cellStyle}>
                <StatusBubble status={row.tocStatus} />
              </TableCell>
              <TableCell sx={cellStyle}>
                <StatusBubble status={row.agreementStatus} />
              </TableCell>
              <TableCell sx={cellStyle}>
                <Link
              ...
              <TableCell sx={lastCellStyle}>
                <Box sx={{ display: "flex", gap: 1 }}>
                  <Button
                    ...
                    color="success"
                    size="small"
                    onClick={() => onVerify(row.email, "Verified")}
                    sx={{ textTransform: "none", fontSize: "0.7rem" }}
                  >
                    Verify
                  </Button>
                  <Button
                    ...
                    color="error"
                    size="small"
                    onClick={() => onVerify(row.email, "Rejected")}
                    sx={{ textTransform: "none", fontSize: "0.7rem" }}
                  >
                    Reject
                  </Button>
...
const VendorManagement = () => {
  ...
  const [agreementData, setAgreementData] = useState<vendorAgreement[]>(
    vendorAgreementData.vendoragreement
  );
  const [registrationData, setRegistrationData] = useState<
    vendorRegistration[]
  >(vendorRegistrationData.vendorRegistration);
  ...
  const handleAgreement = (email: string) => {
    setAgreementData((prevData) =>
      prevData.map((vendor) =>
        vendor.email === email
          ? { ...vendor, agreementStatus: "Fulfilled" }
          : vendor
      )
    );
  };
  ...
  return (
    <div className="min-h-screen bg-[#282828] text-white">
      ...
      <main className="container mx-auto p-8">
        ...
        {/* table */}
        <Paper
          sx={{ mt: 2, p: 1.5, backgroundColor: "#343536", borderRadius: 2 }}
        >
          <Tabs
            value={activeTab}
            onChange={(_, newVal) => handleTabChange(newVal)}
            // 4. Buat props menjadi dinamis
            variant={isMobile ? "scrollable" : "standard"}
            allowScrollButtonsMobile // Biarkan ini agar tombol scroll muncul di mobile
