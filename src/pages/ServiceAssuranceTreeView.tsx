...
const ServiceAssuranceTreeView = () => {
  ...
  function pickByKind(ids: string[], kind: Kind) {
    return ids
      ...
      .filter((n): n is NonNullable<typeof n> => !!n && getKindFromId(n.id) === kind);
  }
  ...
  //zoom slider mui
  const ZoomSlider: React.FC<{ value: number; onChange: (z: number) => void }> = ({ value, onChange }) => {
              ...
              >
                ...
                <div className="flex items-start justify-between px-4 pt-3">
                  <div className="font-semibold">
                    {panelData.rootEnterprise ? (panelData.rootEnterprise.data as any).label : 'Enterprise'}
                  </div>
                </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {panelData.sites.map((n) => (
                          <span
                            ...
                            style={{ border: `1px solid ${(n.data as any).bg}` }}
                          >
                            {(n.data as any).label}
                          </span>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {panelData.core.map((n) => (
                          <span
                            ...
                            className="px-3 py-1 rounded-full text-xs font-medium"
                            style={{
                              backgroundColor: `${(n.data as any).bg}20`,
                              border: `1px solid ${(n.data as any).bg}`,
                            }}
                          >
                            {(n.data as any).label}
                          </span>
